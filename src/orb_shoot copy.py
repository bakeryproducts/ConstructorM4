...
class orb_shoot(QtWidgets.QWidget, Ui_Form):
    ...
    def generatedata(self,x0,y0,xang,xi,yang,yi,mv):
        ...
        from lti import intersection


        file = self.recreate_log()

        h_angs = {'start':x0,'end':xang,'step':xi}
        v_angs = {'start':y0,'end':yang,'step':yi}

        num_shots = int(self.ln_n.text())
        indxs, rays, points = get_shots(h_angs, v_angs, n=num_shots, debug=False)
        shots = combine_rp(rays, points)
        num_rays = rays.shape[0]*2 # because of pair dir-point

        for comp in tqdm(comps):
            norms, thicknesses, polys = self.get_polys(comp)
            num_polys = polys.shape[0]
            polys = polys.reshape((-1,3))

            results = intersection(polys, shots)
